        ...
        public int createOrder(HotelRentOrderSummit order_info, OrderEntities order_queue)
        {
            try
            {
                ...
                foreach (var hotel_rent_item in order_info.obj_hotel_rent)
                {
                    ...
                    //-- Create Hotel Booking Room Extra Packages
                    if(hotel_rent_item.extrapackages!=null && hotel_rent_item.extrapackages.Count > 0)
                    {
                        foreach (var ex in hotel_rent_item.extrapackages)
                        {
                            ex.HotelBookingId= hotel_id;
                            var extrapackages_id = Repository.CreateHotelBookingRoomExtraPackages(ex);
                            if (extrapackages_id < 0)
                            {
                                Telegram.pushLog("APP.CHECKOUT_SERVICE - OrderHotelRentsService - CreateHotelBookingRoomExtraPackages: Cannot Create CreateHotelBookingRoomExtraPackages" + JsonConvert.SerializeObject(ex));
                                Console.WriteLine("APP.CHECKOUT_SERVICE - OrderHotelRentsService - CreateHotelBookingRoomExtraPackages: Cannot Create CreateHotelBookingRoomExtraPackages" + JsonConvert.SerializeObject(ex));
            ...
            try
            {
                ...
                if (data_list != null && data_list.Count > 0)
                {
                    ...
                    foreach (var data in data_list)
                    {
                        ...
                        var extrapackages = new List<HotelBookingRoomExtraPackages>();
